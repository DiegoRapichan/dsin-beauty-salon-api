...
function SlotButton({ slot, selecionado, isAtual, onClick }) {
  ...
  let cls = "relative py-3 rounded-xl text-sm font-black tracking-wider transition-all select-none ";
  let title = hora;

  if (selecionado) {
    cls += "bg-cyan-600 text-white shadow-lg shadow-cyan-900/30 scale-95 ";
  } else if (isLivre) {
    cls += "bg-black border border-white/10 text-gray-300 hover:border-cyan-500/60 hover:text-white active:scale-95 cursor-pointer ";
  } else if (status === "ocupado") {
    cls += "bg-red-950/40 border border-red-900/30 text-red-900 cursor-not-allowed line-through ";
    title = `${hora} — Ocupado`;
  } else if (status === "insuficiente") {
    cls += "bg-yellow-950/30 border border-yellow-900/20 text-yellow-900/60 cursor-not-allowed ";
  ...
  return (
    <button type="button" title={title} disabled={!isLivre}
      ...
      {hora}
      {isAtual && <span className="absolute -top-1.5 -right-1.5 text-[8px] bg-yellow-500 text-black px-1 rounded font-black">atual</span>}
...
export default function EditarAgendamento() {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        ...
        if (!encontrado) { navigate("/meus-agendamentos"); return; }
  ...
  const duracaoTotal = agendamento
    ? agendamento.servicos.reduce((acc, s) => acc + (s.servico?.duracao || 30), 0)
    : 30;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/agendamentos/${id}`, {
        dataHora: new Date(`${dataSelecionada}T${slotSelecionado}:00`).toISOString(),
        isAdmin: false,
      });
      navigate("/meus-agendamentos");
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        ...
        <header className="mb-10">
          <h2 className="text-4xl font-black italic tracking-tighter uppercase">
            REAGENDAR <span className="text-cyan-500">HORÁRIO</span>
          </h2>
          <p className="text-gray-500 text-sm mt-2">
            {agendamento.servicos.map((s) => s.servico.nome).join(", ")}
          </p>
          <p className="text-gray-600 text-xs mt-1">Duração: {duracaoTotal} min</p>
        </header>

        <form onSubmit={handleSubmit} className="bg-[#0a0a0a] p-8 rounded-[3rem] border border-white/5 shadow-2xl space-y-8">
          {/* Data */}
          <div>
            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3 block">Nova data</label>
            <input type="date" min={hoje}
              className="w-full bg-black border border-white/10 p-5 rounded-2xl text-white focus:border-cyan-500 outline-none transition-all font-mono"
          ...
          <div>
            <div className="flex justify-between mb-3">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Horários</label>
              {slots.length > 0 && !carregandoSlots && (
                <span className="text-[10px] text-gray-600">{livres} disponíve{livres === 1 ? "l" : "is"}</span>
              )}
            </div>

            {!dataSelecionada && <p className="text-gray-600 text-xs italic">Selecione uma data primeiro.</p>}

            {dataSelecionada && carregandoSlots && (
              <div className="grid grid-cols-3 gap-2">
                {[...Array(9)].map((_, i) => <div key={i} className="h-10 rounded-xl bg-white/5 animate-pulse" />)}
              </div>
            )}
              ...
              <>
                <div className="grid grid-cols-3 gap-2">
                  {slots.map((slot) => (
                    <SlotButton
                      ...
                      isAtual={slot.hora === horaAtual && dataSelecionada === dataAtual}
                ...
                <div className="flex gap-3 mt-3 flex-wrap">
                  <span className="flex items-center gap-1 text-[10px] text-gray-600">
                    <span className="w-3 h-3 rounded-sm bg-black border border-white/10 inline-block" />Disponível
                  </span>
                  <span className="flex items-center gap-1 text-[10px] text-gray-600">
                    <span className="w-3 h-3 rounded-sm bg-red-950/40 border border-red-900/30 inline-block" />Ocupado
                  </span>
                  <span className="flex items-center gap-1 text-[10px] text-gray-600">
                    <span className="w-3 h-3 rounded-sm bg-yellow-950/30 border border-yellow-900/20 inline-block" />Sem espaço
                  </span>
                </div>
              </>
            )}
          ...
          {erro && (
            <p className="text-red-500 text-xs font-bold bg-red-500/10 border border-red-500/20 p-3 rounded-xl">{erro}</p>
          )}

          <button type="submit" disabled={loading || !slotSelecionado}
            className="w-full bg-cyan-600 hover:bg-cyan-500 disabled:bg-gray-800 disabled:cursor-not-allowed py-5 rounded-2xl font-black uppercase tracking-widest transition-all active:scale-95">
