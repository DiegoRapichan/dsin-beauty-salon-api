...
function SlotButton({ slot, selecionado, onClick }) {
  ...
  let cls =
    "relative py-2.5 rounded-xl text-xs font-black tracking-wider transition-all select-none ";
  let title = hora;
  ...
  if (selecionado) {
    ...
    cls +=
      "bg-black/40 border border-gray-800 text-gray-400 hover:border-pink-500/60 hover:text-white active:scale-95 cursor-pointer ";
  } else if (status === "ocupado") {
    cls +=
      "bg-red-950/30 border border-red-900/20 text-red-900 cursor-not-allowed line-through ";
    title = `${hora} — Ocupado`;
  } else if (status === "insuficiente") {
    cls +=
      "bg-yellow-950/20 border border-yellow-900/10 text-yellow-900/50 cursor-not-allowed ";
...
export default function Agendamento() {
  ...
  useEffect(() => {
    Promise.all([api.get("/clientes"), api.get("/servicos")])
      .then(([resC, resS]) => {
      ...
      <div className="max-w-3xl mx-auto bg-gray-900/40 backdrop-blur-xl p-10 rounded-[2.5rem] border border-gray-800 shadow-2xl">
        <header className="mb-12 text-center">
          <h2 className="text-4xl font-black italic tracking-tighter uppercase">
            NOVO <span className="text-pink-500">HORÁRIO</span>
          </h2>
        ...
        <form onSubmit={salvar} className="space-y-10">
          ...
          <div className="flex flex-col gap-3">
            ...
            <select
              className="bg-black/60 p-4 rounded-2xl border border-gray-800 focus:border-pink-500 outline-none text-white h-14 transition-all"
          ...
          <div className="space-y-4">
              ...
              {servicos.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`relative p-5 rounded-[1.5rem] border-2 transition-all flex flex-col items-center justify-center gap-2 min-h-[110px] ${sel ? "border-pink-500 bg-pink-500/10 text-white" : "border-gray-800 bg-gray-900/40 text-gray-400 hover:border-gray-600"}`}
                  >
                    <span className="font-black text-sm uppercase tracking-tight text-center leading-tight">
                      {s.nome}
                    </span>
                    ...
                    {sel && (
                      <div className="absolute -top-2 -right-2 bg-pink-500 text-black rounded-full w-6 h-6 flex items-center justify-center shadow-lg text-xs font-black">
                        ✓
                      </div>
                    )}
                  </button>
            ...
            {form.servicosSelecionados.length > 0 && (
              <p className="text-[10px] text-pink-500 px-1">
          ...
          <div className="flex flex-col gap-3">
            ...
            <input
              type="date"
              min={hoje}
              className="bg-black/60 p-4 rounded-2xl border border-gray-800 focus:border-pink-500 outline-none text-white h-14 transition-all font-mono"
              ...
              <>
                ...
                <div className="flex gap-4 mt-2 flex-wrap">
                  <span className="flex items-center gap-1.5 text-[10px] text-gray-600">
                    <span className="w-3 h-3 rounded-sm bg-black/40 border border-gray-800 inline-block" />
                    Disponível
                  </span>
                  <span className="flex items-center gap-1.5 text-[10px] text-gray-600">
                    <span className="w-3 h-3 rounded-sm bg-red-950/30 border border-red-900/20 inline-block" />
                    Ocupado
                  </span>
                  <span className="flex items-center gap-1.5 text-[10px] text-gray-600">
                    <span className="w-3 h-3 rounded-sm bg-yellow-950/20 border border-yellow-900/10 inline-block" />
                    Sem espaço
                  </span>
                </div>
              </>
          ...
          <button
            ...
            className="w-full bg-pink-600 hover:bg-pink-500 disabled:bg-gray-800 disabled:cursor-not-allowed py-5 rounded-2xl font-black uppercase tracking-[0.25em] transition-all shadow-xl shadow-pink-900/20 active:scale-95 text-xs"
          >
